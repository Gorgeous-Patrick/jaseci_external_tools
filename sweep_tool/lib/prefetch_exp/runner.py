...
def _is_markov_pooled_policy(policy: str) -> bool:
    return policy == MARKOV_POOLED_PREFIX or re.fullmatch(rf"{MARKOV_POOLED_PREFIX}-n\d+", policy) is not None
...
def _pooled_train_ns(policy: str, options: SweepOptions) -> list[int]:
    match = re.fullmatch(rf"{MARKOV_POOLED_PREFIX}-n(\d+)", policy)
    if match:
...
def _coaccess_pooled_train_ns(policy: str, options: SweepOptions) -> list[int]:
    match = re.fullmatch(rf"{COACCESS_POOLED_PREFIX}-n(\d+)", policy)
    if match:
        return [int(match.group(1))]
...
def _mongo_query_count(adapter) -> str:
    return adapter.mongo_query_count()
