/* Source: /home/patrickli/Space/jaseci_env/jaseci/jac/examples/littleX/components/ExploreTab.cl.jac */
...
function ExploreTab(props) {
  const {allProfiles, followingIds, myProfileId, onFollow, onUnfollow} = props;
  return __jacJsx("div", {}, [(() => {
    let __jac_view_kids = [];
    if (!_jac.builtin.bool(allProfiles)) {
      __jac_view_kids.push(__jacJsx("div", {"className": "lx-loading"}, ["Loading users..."]));
    }
    for (const u of allProfiles) {
      __jac_view_kids.push(__jacJsx("div", {"className": "lx-explore-item", "key": u.id}, [__jacJsx("img", {"className": "lx-avatar", "src": ("https://i.pravatar.cc/150?u=fake@" + u.username), "alt": u.username}, []), __jacJsx("div", {"className": "lx-explore-info"}, [__jacJsx("div", {"className": "lx-explore-name"}, [(u.username.includes("@") ? u.username.split("@")[0] : u.username)]), __jacJsx("div", {"className": "lx-explore-handle"}, [("@" + (u.username.includes("@") ? u.username.split("@")[0] : u.username))]), __jacJsx("div", {"className": "lx-explore-bio"}, [(u.bio || "No bio yet")])]), (() => {
        let __jac_view_kids_1 = [];
        if (followingIds.includes(u.id)) {
          __jac_view_kids_1.push(__jacJsx("button", {"className": "lx-unfollow-btn", "onClick": () => {
            onUnfollow(u.id);
          }}, ["Following"]));
        } else if ((u.id !== myProfileId)) {
          __jac_view_kids_1.push(__jacJsx("button", {"className": "lx-follow-btn", "onClick": () => {
            onFollow(u.id);
          }}, ["Follow"]));
        }
...
export {ExploreTab};
if (typeof globalThis !== "undefined") { if (!globalThis.__jacEndpointEffects__) globalThis.__jacEndpointEffects__ = {}; Object.assign(globalThis.__jacEndpointEffects__, {"func:ExploreTab": {"is_writer": false, "touches": ["*"]}}); };
