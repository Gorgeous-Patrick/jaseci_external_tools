/* Source: /home/patrickli/Space/jaseci_env/jaseci/jac/examples/littleX/components/ProfileTab.cl.jac */
...
function ProfileTab(props) {
  const {myProfile, displayUsername, profileAvatar, profileUsername, editingBio, bioText, onBioChange, onSaveBio, onCancelEditBio, onLike, onComment, onDelete, onRepost} = props;
  let followingCount = ((myProfile && myProfile.following) ? myProfile.following.length : 0);
  let followerCount = ((myProfile && myProfile.followers) ? myProfile.followers.length : 0);
  let postCount = ((myProfile && myProfile.tweets) ? myProfile.tweets.length : 0);
  let myTweets = ((myProfile && myProfile.tweets) ? myProfile.tweets : []);
  return __jacJsx("div", {}, [__jacJsx("div", {"className": "lx-profile-banner"}, []), __jacJsx("div", {"className": "lx-profile-info"}, [__jacJsx("div", {"className": "lx-profile-avatar-wrap"}, [__jacJsx("img", {"className": "lx-avatar-lg", "src": profileAvatar, "alt": profileUsername}, [])]), __jacJsx("div", {"className": "lx-profile-display-name"}, [displayUsername]), __jacJsx("div", {"className": "lx-profile-handle"}, [("@" + displayUsername)]), (() => {
    let __jac_view_kids = [];
    if (editingBio) {
      __jac_view_kids.push(__jacJsx("div", {"className": "lx-bio-edit-wrap"}, [__jacJsx("textarea", {"className": "lx-bio-input", "rows": 3, "placeholder": "Write your bio...", "value": bioText, "onChange": e => {
        onBioChange(e.target.value);
      }}, []), __jacJsx("div", {"style": {"display": "flex", "gap": "0.5rem"}}, [__jacJsx("button", {"className": "lx-bio-save-btn", "onClick": () => {
        onSaveBio();
      }}, ["Save"]), __jacJsx("button", {"className": "lx-edit-profile-btn", "onClick": () => {
        onCancelEditBio();
      }}, ["Cancel"])])]));
    } else {
      __jac_view_kids.push(__jacJsx("div", {"className": "lx-profile-bio-text"}, [((myProfile && myProfile.bio) ? myProfile.bio : "No bio yet")]));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })(), __jacJsx("div", {"className": "lx-profile-stats"}, [__jacJsx("div", {"className": "lx-profile-stat"}, [__jacJsx("strong", {}, [String(followingCount)]), "Following"]), __jacJsx("div", {"className": "lx-profile-stat"}, [__jacJsx("strong", {}, [String(followerCount)]), ((followerCount === 1) ? "Follower" : "Followers")]), __jacJsx("div", {"className": "lx-profile-stat"}, [__jacJsx("strong", {}, [String(postCount)]), ((postCount === 1) ? "Post" : "Posts")])])]), (() => {
    let __jac_view_kids_1 = [];
    for (const tweet of myTweets) {
      __jac_view_kids_1.push(__jacJsx(TweetCard, {"key": tweet.id, "tweet": tweet, "myUsername": profileUsername, "onLike": onLike, "onComment": onComment, "onDelete": onDelete, "onRepost": onRepost}, []));
    }
    return __jacJsx(null, {}, [__jac_view_kids_1]);
  })()]);
}
export {ProfileTab};
if (typeof globalThis !== "undefined") { if (!globalThis.__jacEndpointEffects__) globalThis.__jacEndpointEffects__ = {}; Object.assign(globalThis.__jacEndpointEffects__, {"func:ProfileTab": {"is_writer": false, "touches": ["*"]}}); };
