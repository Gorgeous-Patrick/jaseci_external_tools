/* Source: /home/patrickli/Space/jaseci_env/jaseci/jac/examples/littleX/components/Composer.cl.jac */
...
function Composer(props) {
  const {avatar, avatarAlt, placeholder, value, onChange, onPost, rows: rows = 3, buttonLabel: buttonLabel = "Post", showCharRing: showCharRing = false} = props;
  let charsLeft = (280 - value.length);
  let ring_progress = Math.min(1.0, ((280 - charsLeft) / 280.0));
  let ring_offset = String((62.83 * (1.0 - ring_progress)));
  let ring_color = ((charsLeft < 0) ? "#ef4444" : ((charsLeft < 50) ? "#f97316" : ((charsLeft < 100) ? "#fb923c" : "#2a2a35")));
  let over_limit = (showCharRing && (charsLeft < 0));
  return __jacJsx("div", {"className": "lx-composer"}, [__jacJsx("img", {"className": "lx-avatar", "src": avatar, "alt": avatarAlt}, []), __jacJsx("div", {"className": "lx-composer-body"}, [__jacJsx("textarea", {"className": "lx-composer-textarea", "placeholder": placeholder, "value": value, "onChange": e => {
    onChange(e.target.value);
  }, "rows": rows}, []), __jacJsx("div", {"className": "lx-composer-footer"}, [(() => {
    let __jac_view_kids = [];
    if (showCharRing) {
      __jac_view_kids.push(__jacJsx("div", {"className": "lx-char-ring-wrap"}, [__jacJsx("svg", {"className": "lx-char-ring", "viewBox": "0 0 24 24"}, [__jacJsx("circle", {"className": "lx-char-ring-bg", "cx": 12, "cy": 12, "r": 10}, []), __jacJsx("circle", {"className": "lx-char-ring-fill", "cx": 12, "cy": 12, "r": 10, "strokeDasharray": "62.83", "strokeDashoffset": ring_offset, "stroke": ring_color}, [])]), (() => {
        let __jac_view_kids_1 = [];
        if ((charsLeft <= 20)) {
          __jac_view_kids_1.push(__jacJsx("span", {"className": ((charsLeft < 0) ? "lx-char-count-over" : "lx-char-count-warn")}, [String(charsLeft)]));
        }
        return __jacJsx(null, {}, [__jac_view_kids_1]);
      })()]));
    } else {
      __jac_view_kids.push(__jacJsx("div", {}, []));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })(), __jacJsx("button", {"className": "lx-post-btn", "disabled": (!value.trim() || over_limit), "onClick": () => {
    onPost();
  }}, [buttonLabel])])])]);
}
export {Composer};
if (typeof globalThis !== "undefined") { if (!globalThis.__jacEndpointEffects__) globalThis.__jacEndpointEffects__ = {}; Object.assign(globalThis.__jacEndpointEffects__, {"func:Composer": {"is_writer": false, "touches": ["*"]}}); };
