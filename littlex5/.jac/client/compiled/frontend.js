/* Source: /home/patrickli/Space/jaseci_env/jaseci/jac/examples/littleX/frontend.cl.jac */
...
async function setup_profile() {
  return await __jacCallFunction("setup_profile", {});
}
async function get_profile() {
  return await __jacCallFunction("get_profile", {});
}
async function get_all_profiles() {
  return await __jacCallFunction("get_all_profiles", {});
}
async function follow_user() {
  return await __jacCallFunction("follow_user", {});
}
async function unfollow_user() {
  return await __jacCallFunction("unfollow_user", {});
}
async function create_tweet() {
  return await __jacCallFunction("create_tweet", {});
}
async function delete_tweet() {
  return await __jacCallFunction("delete_tweet", {});
}
async function like_tweet() {
  return await __jacCallFunction("like_tweet", {});
}
async function add_comment() {
  return await __jacCallFunction("add_comment", {});
}
async function load_feed() {
  return await __jacCallFunction("load_feed", {});
}
async function get_trending() {
  return await __jacCallFunction("get_trending", {});
}
async function create_channel() {
  return await __jacCallFunction("create_channel", {});
}
async function join_channel() {
  return await __jacCallFunction("join_channel", {});
}
async function leave_channel() {
  return await __jacCallFunction("leave_channel", {});
}
async function get_channels() {
  return await __jacCallFunction("get_channels", {});
}
async function get_channel_detail() {
  return await __jacCallFunction("get_channel_detail", {});
}
async function create_channel_tweet() {
  return await __jacCallFunction("create_channel_tweet", {});
}
async function UserView() {
  return await __jacCallFunction("UserView", {});
}
async function TweetView() {
  return await __jacCallFunction("TweetView", {});
}
async function ChannelView() {
  return await __jacCallFunction("ChannelView", {});
}
async function TrendingTag() {
  return await __jacCallFunction("TrendingTag", {});
}
async function ProfileView() {
  return await __jacCallFunction("ProfileView", {});
}
const _jac = {
  int: {
    bit_length(n) {
...
function app() {
  ...
  useEffect(() => {
    if (isLoggedIn) {
      setShowWelcome(!localStorage.getItem("lx_welcomed"));
      loadData();
    }
  }, [isLoggedIn]);
  async function loadData() {
    let result = await __jacSpawn("setup_profile", "", {"username": (localStorage.getItem("lx_username") || "")});
    setMyProfile((_jac.builtin.bool(result.reports) ? result.reports[0] : null));
    let feedResult = await __jacSpawn("load_feed", "", {"search_query": ""});
    setTweets((_jac.builtin.bool(feedResult.reports) ? feedResult.reports[0] : []));
    setFeedLoading(false);
    let profileResult = await __jacSpawn("get_profile", "", {});
    setMyProfile((_jac.builtin.bool(profileResult.reports) ? profileResult.reports[0] : myProfile));
    let profilesResult = await __jacSpawn("get_all_profiles", "", {});
    setAllProfiles((_jac.builtin.bool(profilesResult.reports) ? profilesResult.reports[0] : []));
    let trendResult = await __jacSpawn("get_trending", "", {});
    setTrendingTags((_jac.builtin.bool(trendResult.reports) ? trendResult.reports[0] : []));
    let total_activity = _jac.builtin.sum((((myProfile && myProfile.tweets) ? myProfile.tweets : [])).map(t => (t.likes.length + t.comments.length)));
    let last_seen_str = localStorage.getItem("lx_activity_total");
    let last_seen = (last_seen_str ? Math.trunc(Number(last_seen_str)) : 0);
    setNotifCount(((total_activity > last_seen) ? (total_activity - last_seen) : 0));
  }
  async function handleLogin() {
    ...
    setAuthLoading(true);
    let result = await jacLogin(authUsername, authPassword);
  ...
  async function handleSignup() {
    ...
    setAuthLoading(true);
    let result = await jacSignup(authUsername, authPassword);
    if (result["success"]) {
      let loginResult = await jacLogin(authUsername, authPassword);
      setAuthLoading(false);
      if (loginResult) {
        localStorage.setItem("lx_username", authUsername);
        setIsLoggedIn(true);
      ...
      setAuthLoading(false);
      setAuthError(String((result["error"] ? result["error"] : "Signup failed")));
    }
  }
  ...
  async function postTweet() {
    ...
    setComposerText("");
    await __jacSpawn("create_tweet", "", {"content": text});
    let feedResult = await __jacSpawn("load_feed", "", {"search_query": ""});
    setTweets((_jac.builtin.bool(feedResult.reports) ? feedResult.reports[0] : []));
    let profileResult = await __jacSpawn("get_profile", "", {});
    setMyProfile((_jac.builtin.bool(profileResult.reports) ? profileResult.reports[0] : myProfile));
  ...
  async function handleLike(tweetId) {
    await __jacSpawn("like_tweet", "", {"tweet_id": tweetId});
    let feedResult = await __jacSpawn("load_feed", "", {"search_query": searchQuery});
    setTweets((_jac.builtin.bool(feedResult.reports) ? feedResult.reports[0] : []));
    if (myProfile) {
      let profileResult = await __jacSpawn("get_profile", "", {});
      setMyProfile((_jac.builtin.bool(profileResult.reports) ? profileResult.reports[0] : myProfile));
    ...
    if (selectedChannel) {
      let detailResult = await __jacSpawn("get_channel_detail", "", {"channel_id": selectedChannel.id});
      let detail = (_jac.builtin.bool(detailResult.reports) ? detailResult.reports[0] : null);
      if (detail) {
        setSelectedChannel(detail);
        setChannelPosts(_jac.poly.or_(detail.posts, () => []));
      }
    }
  }
  async function handleComment(tweetId, content) {
    await __jacSpawn("add_comment", "", {"tweet_id": tweetId, "content": content});
    let feedResult = await __jacSpawn("load_feed", "", {"search_query": searchQuery});
    setTweets((_jac.builtin.bool(feedResult.reports) ? feedResult.reports[0] : []));
    if (selectedChannel) {
      let detailResult = await __jacSpawn("get_channel_detail", "", {"channel_id": selectedChannel.id});
  ...
  async function handleDelete(tweetId) {
    await __jacSpawn("delete_tweet", "", {"tweet_id": tweetId});
    let feedResult = await __jacSpawn("load_feed", "", {"search_query": searchQuery});
    setTweets((_jac.builtin.bool(feedResult.reports) ? feedResult.reports[0] : []));
    let profileResult = await __jacSpawn("get_profile", "", {});
    setMyProfile((_jac.builtin.bool(profileResult.reports) ? profileResult.reports[0] : myProfile));
  ...
  async function handleFollow(profileId) {
    ...
    await __jacSpawn("follow_user", "", {"target_id": profileId});
    let profileResult = await __jacSpawn("get_profile", "", {});
    setMyProfile((_jac.builtin.bool(profileResult.reports) ? profileResult.reports[0] : myProfile));
    let feedResult = await __jacSpawn("load_feed", "", {"search_query": searchQuery});
    setTweets((_jac.builtin.bool(feedResult.reports) ? feedResult.reports[0] : []));
  ...
  async function handleUnfollow(profileId) {
    await __jacSpawn("unfollow_user", "", {"target_id": profileId});
    let profileResult = await __jacSpawn("get_profile", "", {});
    setMyProfile((_jac.builtin.bool(profileResult.reports) ? profileResult.reports[0] : myProfile));
    let feedResult = await __jacSpawn("load_feed", "", {"search_query": searchQuery});
    setTweets((_jac.builtin.bool(feedResult.reports) ? feedResult.reports[0] : []));
  ...
  async function handleSaveBio() {
    let uname = (myProfile ? myProfile.username : "");
    await __jacSpawn("setup_profile", "", {"username": uname, "bio": bioText});
    let profileResult = await __jacSpawn("get_profile", "", {});
    setMyProfile((_jac.builtin.bool(profileResult.reports) ? profileResult.reports[0] : myProfile));
    setEditingBio(false);
  }
  async function handleSearch(query) {
    let feedResult = await __jacSpawn("load_feed", "", {"search_query": query});
    setTweets((_jac.builtin.bool(feedResult.reports) ? feedResult.reports[0] : []));
  }
  function handleRepost(tweetId, content, author) {
    let display_author = (author.includes("@") ? author.split("@")[0] : author);
    setComposerText(((("RT @" + display_author) + ": ") + content));
    setActiveTab("feed");
  }
  ...
  function navigate(tab) {
    ...
    if ((tab === "profile")) {
      let total_activity = _jac.builtin.sum((((myProfile && myProfile.tweets) ? myProfile.tweets : [])).map(t => (t.likes.length + t.comments.length)));
      localStorage.setItem("lx_activity_total", String(total_activity));
      setNotifCount(0);
    }
  }
  async function loadChannels() {
    setChannelsLoading(true);
    let result = await __jacSpawn("get_channels", "", {});
    setChannels((_jac.builtin.bool(result.reports) ? result.reports[0] : []));
    setChannelsLoading(false);
  }
  async function handleCreateChannel(name, description) {
    if (!name.trim()) {
      return;
    }
    await __jacSpawn("create_channel", "", {"name": name.trim(), "description": description.trim()});
    let result = await __jacSpawn("get_channels", "", {});
    setChannels((_jac.builtin.bool(result.reports) ? result.reports[0] : []));
  }
  async function handleJoinChannel(channelId) {
    await __jacSpawn("join_channel", "", {"channel_id": channelId});
    let result = await __jacSpawn("get_channels", "", {});
    setChannels((_jac.builtin.bool(result.reports) ? result.reports[0] : []));
    if ((selectedChannel && (selectedChannel.id === channelId))) {
      let detailResult = await __jacSpawn("get_channel_detail", "", {"channel_id": channelId});
      let detail = (_jac.builtin.bool(detailResult.reports) ? detailResult.reports[0] : null);
      if (detail) {
        setSelectedChannel(detail);
        setChannelPosts(_jac.poly.or_(detail.posts, () => []));
  ...
  async function handleLeaveChannel(channelId) {
    await __jacSpawn("leave_channel", "", {"channel_id": channelId});
    let result = await __jacSpawn("get_channels", "", {});
    setChannels((_jac.builtin.bool(result.reports) ? result.reports[0] : []));
    if ((selectedChannel && (selectedChannel.id === channelId))) {
      let detailResult = await __jacSpawn("get_channel_detail", "", {"channel_id": channelId});
  ...
  async function handleChannelPost(text) {
    if ((!text.trim() || !selectedChannel)) {
      return;
    }
    await __jacSpawn("create_channel_tweet", "", {"channel_id": selectedChannel.id, "content": text});
    let detailResult = await __jacSpawn("get_channel_detail", "", {"channel_id": selectedChannel.id});
    let detail = (_jac.builtin.bool(detailResult.reports) ? detailResult.reports[0] : null);
    if (detail) {
      setSelectedChannel(detail);
  ...
  if (checkingAuth) {
    return __jacJsx("div", {"className": "lx-loading"}, ["Loading..."]);
  }
  if (!isLoggedIn) {
    return __jacJsx(AuthForm, {"isSignup": isSignup, "username": authUsername, "password": authPassword, "error": authError, "loading": authLoading, "onUsernameChange": e => {
  ...
  let profileUsername = (myProfile ? myProfile.username : "");
  let displayUsername = (profileUsername.includes("@") ? profileUsername.split("@")[0] : profileUsername);
  let profileAvatar = ("https://i.pravatar.cc/150?u=fake@" + profileUsername);
  let myProfileId = (myProfile ? myProfile.id : "");
  let followingIds = (((myProfile && myProfile.following) ? myProfile.following : [])).map(f => f.id);
  let toFollow = allProfiles.filter(u => (!followingIds.includes(u.id) && (u.id !== myProfileId))).map(u => u);
  return __jacJsx("div", {"className": "lx-root"}, [__jacJsx(Sidebar, {"activeTab": activeTab, "displayUsername": displayUsername, "profileUsername": profileUsername, "profileAvatar": profileAvatar, "notifCount": notifCount, "onNavigate": navigate, "onLogout": handleLogout}, []), __jacJsx("main", {"className": "lx-main"}, [__jacJsx("div", {"className": "lx-feed-header"}, [__jacJsx("span", {"className": "lx-feed-title"}, [((activeTab === "feed") ? "Home" : ((activeTab === "explore") ? "Explore" : ((activeTab === "channels") ? "Channels" : "Profile")))]), (() => {
    let __jac_view_kids = [];
    if ((activeTab === "profile")) {
      __jac_view_kids.push(__jacJsx("button", {"className": "lx-edit-profile-btn", "onClick": () => {
        setEditingBio(!editingBio);
        setBioText(((myProfile && myProfile.bio) ? myProfile.bio : ""));
      }}, [(editingBio ? "Cancel" : "Edit profile")]));
    }
    return __jacJsx(null, {}, [__jac_view_kids]);
  })()]), (() => {
    let __jac_view_kids_1 = [];
    if ((activeTab === "feed")) {
      __jac_view_kids_1.push(__jacJsx(FeedTab, {"showWelcome": showWelcome, "onDismissWelcome": handleDismissWelcome, "profileAvatar": profileAvatar, "profileUsername": profileUsername, "composerText": composerText, "onComposerChange": v => {
        setComposerText(v);
      }, "onPost": postTweet, "feedLoading": feedLoading, "tweets": tweets, "onLike": handleLike, "onComment": handleComment, "onDelete": handleDelete, "onRepost": handleRepost}, []));
    } else if ((activeTab === "explore")) {
      __jac_view_kids_1.push(__jacJsx(ExploreTab, {"allProfiles": allProfiles, "followingIds": followingIds, "myProfileId": myProfileId, "onFollow": handleFollow, "onUnfollow": handleUnfollow}, []));
    } else if ((activeTab === "channels")) {
      __jac_view_kids_1.push(__jacJsx(ChannelsTab, {"channels": channels, "channelsLoading": channelsLoading, "selectedChannel": selectedChannel, "channelPosts": channelPosts, "profileAvatar": profileAvatar, "profileUsername": profileUsername, "onCreateChannel": handleCreateChannel, "onJoinChannel": handleJoinChannel, "onLeaveChannel": handleLeaveChannel, "onSelectChannel": handleSelectChannel, "onChannelPost": handleChannelPost, "onBackToChannels": handleBackToChannels, "onLike": handleLike, "onComment": handleComment, "onDelete": handleDelete, "onRepost": handleRepost}, []));
    } else if ((activeTab === "profile")) {
      __jac_view_kids_1.push(__jacJsx(ProfileTab, {"myProfile": myProfile, "displayUsername": displayUsername, "profileAvatar": profileAvatar, "profileUsername": profileUsername, "editingBio": editingBio, "bioText": bioText, "onBioChange": v => {
        setBioText(v);
      }, "onSaveBio": handleSaveBio, "onCancelEditBio": () => {
        setEditingBio(false);
      }, "onLike": handleLike, "onComment": handleComment, "onDelete": handleDelete, "onRepost": handleRepost}, []));
    }
    return __jacJsx(null, {}, [__jac_view_kids_1]);
  })()]), __jacJsx(RightSidebar, {"searchQuery": searchQuery, "onSearchChange": v => {
    setSearchQuery(v);
  }, "onSearch": handleSearch, "onClearSearch": () => {
    setSearchQuery("");
    handleSearch("");
  }, "trendingTags": trendingTags, "onTrendClick": handleTrendClick, "toFollow": toFollow, "onFollow": handleFollow, "onShowMore": () => {
    navigate("explore");
  }}, []), __jacJsx(MobileNav, {"activeTab": activeTab, "notifCount": notifCount, "onNavigate": navigate}, []), __jacJsx("button", {"className": "lx-mobile-fab", "onClick": () => {
    navigate("feed");
  }}, [__jacJsx(Feather, {"size": 22}, [])])]);
}
export {app};
