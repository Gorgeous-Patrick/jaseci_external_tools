#!/usr/bin/env python3
"""Plot sweep_results_e2e.csv profiling data for linked_list."""
...
def main():
    ...
    numeric_cols = [
        "list_size", "trial", "e2e_ms", "ttg_total_ms", "topo_idx_ms",
    ...
    grouped = df.groupby(["list_size", "ttg_enabled"]).mean(numeric_only=True).reset_index()

    ttg_on  = grouped[grouped["ttg_enabled"] == "enabled"].sort_values("list_size")
    ttg_off = grouped[grouped["ttg_enabled"] == "disabled"].sort_values("list_size")
    ...
    ax.set_xlabel("List Size (# of items)")
    ax.set_ylabel("Time (ms)")
    ax.set_xticks(x)
    ax.set_xticklabels(ttg_on["list_size"].astype(int).values)
    ax.set_title("E2E Time: TTG vs No TTG (Linked List Traverse)")
    ax.legend()
    ax.grid(axis="y", alpha=0.3)

    # --- Plot 2: TTG internal breakdown ---
    if has_breakdown:
        ax2 = axes[1]
        ...
        ax2.set_xlabel("List Size (# of items)")
        ax2.set_ylabel("TTG Generator time (ms)")
        ax2.set_xticks(x)
        ax2.set_xticklabels(ttg_on["list_size"].astype(int).values)
